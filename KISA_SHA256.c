...
void SHA256_Process( OUT SHA256_INFO *Info, IN const BYTE *pszMessage, IN UINT uDataLen )
{
	...
	{
		// 64비트만큼 szBuffer로 pszMessage를 copy 
		...
		// 알고리즘 진행한 만큼 pszMessage 포인터 이동
		pszMessage += (SHA256_DIGEST_BLOCKLEN - remain_buffer);
		// 알고리즘 진행한 만큼 uDataLen 감소
...
void SHA256_Close( OUT SHA256_INFO *Info, IN BYTE *pszDigest )
{
	...
	// 
	((ULONG_PTR)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 2] = Info->uHighLength; // 14
	((ULONG_PTR)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 1] = Info->uLowLength;  // 15
